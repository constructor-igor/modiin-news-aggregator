    ...
    public class GoogleTrafficProducerTests
    {
        ...
        public void GetMessage_2Traffics()
        {
            using (Stream htmlStream = File.OpenRead(@"..\..\Data\GoogleTrafficRehovotModiin.html"))
            {
                ...
                Assert.AreEqual("In current traffic: 28 mins (Standard: 25 mins), In current traffic: 38 mins (Standard: 32 mins)", message.Text);
                Assert.AreEqual(2, message.SuggestedTraffic.Count);
                Assert.AreEqual("   In current traffic: 28 mins   ", message.SuggestedTraffic[0].InCurrentTraffic);
                Assert.AreEqual("   In current traffic: 38 mins   ", message.SuggestedTraffic[1].InCurrentTraffic);
            }
        }
    }
}